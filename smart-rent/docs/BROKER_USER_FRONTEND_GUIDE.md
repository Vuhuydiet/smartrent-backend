...
2. **Upload documents** — user uploads 3 identity images using the presigned URL flow.
3. **Submit registration** — `POST /v1/users/broker/register` with the 3 media IDs.
...
| 3 | `certMediaId` | Practising certificate |
...
### Step 1 — Upload a Document (repeat ×3)
...
### Step 3 — Confirm Upload (repeat ×3)
...
| Step | What to show |
|---|---|
| Before upload | 3 file inputs labelled CCCD Front, CCCD Back, Certificate |
...
Show thumbnail/link for each document on the profile page. URLs come from `cccdFrontUrl`, `cccdBackUrl`, `certUrl` in the status response. These are presigned and short-lived (regenerated each time status is fetched).
...
| Re-apply after rejection | Must re-select all 3 documents (new media IDs required) |
...
| Cert label | "Practising Certificate" | "Chứng chỉ hành nghề" |
...
Document URL fields (`cccdFrontUrl`, etc.) are absent in older backend versions. For certificate URL, support both new and legacy keys:
```typescript
const hasDocs = status?.cccdFrontUrl != null;
const certUrl = status?.certUrl ?? (status as any)?.certFrontUrl ?? null;
```
...
| # | Test | Pass |
|---|---|---|
| 1 | Upload 3 docs → submit → status = PENDING | [ ] |
