...
-- =====================================================
-- 2. UPDATE TRANSACTIONS TABLE - Remove wallet fields
-- =====================================================

-- Drop balance_before and balance_after columns (no wallet anymore)
...
-- Update transaction_type enum to remove WALLET_TOPUP
ALTER TABLE transactions 
...
-- =====================================================
-- 3. UPDATE PUSH_HISTORY TABLE - Add transaction tracking
-- =====================================================

-- Add transaction_id to track pay-per-boost
ALTER TABLE push_history 
ADD COLUMN transaction_id VARCHAR(36) AFTER user_benefit_id;

-- Add foreign key constraint
ALTER TABLE push_history 
ADD CONSTRAINT fk_ph_transaction 
FOREIGN KEY (transaction_id) REFERENCES transactions(transaction_id) ON DELETE SET NULL;

-- Add index for transaction lookups
ALTER TABLE push_history 
ADD INDEX idx_ph_transaction_id (transaction_id);
...
ALTER TABLE push_history 
MODIFY COLUMN transaction_id VARCHAR(36) 
COMMENT 'Transaction ID if boost was paid via DIRECT_PAYMENT';
