...
public class OpenAPIConfig {
    ...
    private Components buildComponents() {
        return new Components()
                ...
                // Pagination related schemas (for listings and other paged endpoints)
                .addSchemas("ListingPageData", new Schema<>()
                        .type("object")
                        .description("Paginated listing data wrapper")
                        .addProperty("content", new Schema<>()
                                .type("array")
                                .items(new Schema<>().$ref("#/components/schemas/ListingResponse"))
                                .description("Array of listing items"))
                        .addProperty("totalElements", new Schema<>().type("integer").example(1).description("Total number of elements matching criteria"))
                        .addProperty("totalPages", new Schema<>().type("integer").example(1).description("Total number of pages"))
                        .addProperty("page", new Schema<>().type("integer").example(0).description("Current page index (0-based)"))
                        .addProperty("size", new Schema<>().type("integer").example(20).description("Page size"))
                )
                .addSchemas("ListingPageApiResponse", new Schema<>()
                        .type("object")
                        .description("Standard API wrapper for paginated listings")
                        .addProperty("code", new Schema<>().type("string").example("200"))
                        .addProperty("message", new Schema<>().type("string").example("Filter listings successfully"))
                        .addProperty("data", new Schema<>().$ref("#/components/schemas/ListingPageData"))
                );
    }
