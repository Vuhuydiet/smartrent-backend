...
public class ModerationEmailBuilder {
    ...
    public static String buildApprovedEmail(String listingTitle, String ownerFirstName) {
        ...
        html.append("Tin đăng <strong>%s</strong> của bạn đã được duyệt và hiện đã hiển thị công khai trên nền tảng.".formatted(safe(listingTitle)));
        html.append("</div>");
        html.append("<div class=\"card card-success\">");
    ...
    public static String buildRejectedEmail(String listingTitle, String ownerFirstName, String reason) {
        ...
        html.append("<div class=\"greeting\">Xin chào %s,</div>".formatted(safe(ownerFirstName)));
        html.append("<div class=\"message\">");
        html.append("Rất tiếc, tin đăng <strong>%s</strong> của bạn đã bị từ chối sau khi được đội ngũ kiểm duyệt xem xét.".formatted(safe(listingTitle)));
        html.append("</div>");
        if (reason != null && !reason.isBlank()) {
            html.append("<div class=\"card card-danger\">");
            html.append("<div class=\"card-title\">Lý do từ chối</div>");
    ...
    public static String buildRevisionRequestedEmail(String listingTitle, String ownerFirstName, String reason) {
        ...
        html.append("</div>");
        if (reason != null && !reason.isBlank()) {
            html.append("<div class=\"card card-warning\">");
            html.append("<div class=\"card-title\">Nội dung cần cập nhật</div>");
            html.append("<div class=\"card-body\">%s</div>".formatted(safe(reason)));
            html.append("</div>");
        }
    ...
    public static String buildReportActionRequiredEmail(String listingTitle, String ownerFirstName, String adminNotes) {
        ...
        html.append("<div class=\"container\">");
        html.append("<div class=\"header\"><h1>Yêu cầu hành động</h1></div>");
        html.append("<div class=\"content\">");
        html.append("<div class=\"greeting\">Xin chào %s,</div>".formatted(safe(ownerFirstName)));
        html.append("<div class=\"message\">");
        ...
        if (adminNotes != null && !adminNotes.isBlank()) {
            html.append("<div class=\"card card-warning\">");
            html.append("<div class=\"card-title\">Ghi chú từ quản trị viên</div>");
    ...
    public static String buildReportResolvedForReporterEmail(String listingTitle, String reporterName,
        ...
        String headerText = resolved ? "Báo cáo đã được xử lý" : "Báo cáo đã được xem xét";
        ...
        String statusClass = resolved ? "card card-success" : "card card-info";
        String statusBody = resolved
                ? "Đã xử lý &mdash; biện pháp đã được áp dụng."
                : "Đã xem xét &mdash; không cần hành động bổ sung.";
        ...
        html.append("</div>");
        html.append("<div class=\"%s\">".formatted(statusClass));
        html.append("<div class=\"card-title\">Kết quả</div>");
        html.append("<div class=\"card-body\">%s</div>".formatted(statusBody));
        html.append("</div>");
        if (adminNotes != null && !adminNotes.isBlank()) {
            html.append("<div class=\"card card-accent\">");
            html.append("<div class=\"card-title\">Ghi chú từ quản trị viên</div>");
    ...
    public static String buildReportResolvedForOwnerEmail(String listingTitle, String ownerFirstName,
                                                            String resolution, String adminNotes) {
        boolean resolved = "RESOLVED".equalsIgnoreCase(resolution);
        String headerText = resolved ? "Báo cáo về tin đăng của bạn — Đã xử lý" : "Báo cáo về tin đăng của bạn — Không phát hiện vấn đề";
        String outcomeText = resolved
        ...
        String statusClass = resolved ? "card card-warning" : "card card-success";
        String statusBody = resolved
                ? "Đã xử lý &mdash; vui lòng kiểm tra lại tin đăng của bạn."
                : "Đã xem xét &mdash; không phát hiện vấn đề.";

        StringBuilder html = EmailBuilder.buildHtmlHeader(headerText);
        html.append("<div class=\"container\">");
        html.append("<div class=\"header\"><h1>Báo cáo về tin đăng của bạn</h1></div>");
        html.append("<div class=\"content\">");
        html.append("<div class=\"greeting\">Xin chào %s,</div>".formatted(safe(ownerFirstName)));
        html.append("<div class=\"message\">");
